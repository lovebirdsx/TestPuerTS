...
export interface Connection<TContext> extends Client<TContext> {
	readonly connectionEvent: ClientConnectionEvent;
	...
	constructor(onDidClientConnect: Event<ClientConnectionEvent>) {
		onDidClientConnect((connectionEvent) => {
			...
			onFirstMessage((msg) => {
				...
				const connection: Connection<TContext> = { connectionEvent, channelServer, channelClient, ctx };
	...
	export function fromService<TContext>(service: unknown, options?: ICreateServiceChannelOptions): IServerChannel<TContext> {
		...
		return new (class implements IServerChannel {
			listen<T>(_: unknown, event: string, arg: any): Event<T> {
				...
				if (isProxyService) {
					handler[event] = mapEventNameToEvent.set(event, Event.buffer(handler[event] as Event<unknown>, true));
