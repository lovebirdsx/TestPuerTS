...
export namespace SocketDiagnostics {
	...
	function getSocketId(nativeObject: any, _label: string): string {
...
export class NetIPCClient<TContext = string> extends IPCClient<TContext> {
	static fromSocket<TContext = string>(socket: ISocket, id: TContext): NetIPCClient<TContext> {
		return new NetIPCClient(new Protocol(socket), id);
	}
