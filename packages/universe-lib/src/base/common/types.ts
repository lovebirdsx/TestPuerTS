...
export function validateConstraints(args: unknown[], constraints: (TypeConstraint | undefined)[]): void {
...
export type Ctor<T extends TObject = TObject> = new (...args: any[]) => T;
export type TObject = Exclude<object, any[]>;
...
export interface ISymbolShape {
	id: string;
	access: EAccess;
}

export interface ISymbolFileShape extends ISymbolShape {
	namespace?: string;
}

export type TEnum<T extends string | number = string | number> = { [key: string]: T };
...
export type BoolResult = { ok: true } | ResultError;

export interface IFileItem<T extends TObject = TObject> {
	uri: URI;
	object: T;
}

export interface IVsPlayObjectBase {
	id: string;
	type: string;
	access: EAccess;
	namespace?: string;
...
export type IVsPlayFileItem = IFileItem<IVsPlayObjectBase>;

export type ClassTypeInfo = { type: 'class'; ctor: Ctor; base?: ClassTypeInfo };

export type TypeInfo =
	| ClassTypeInfo //
	| { type: 'none' }
	| { type: 'enum'; enum: TEnum }
	| { type: 'int32' }
	| { type: 'int64' }
	| { type: 'float' }
	| { type: 'double' }
