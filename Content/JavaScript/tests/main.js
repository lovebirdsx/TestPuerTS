...
globalThis.setTimeout = function(handler, timeout2, ...args) {
  return _origSetTimeout(handler, timeout2 ?? 0, ...args);
};
globalThis.setInterval = function(handler, timeout2, ...args) {
  return _origSetInterval(handler, timeout2 ?? 0, ...args);
};
  ...
  return new Promise((resolve2, reject) => {
    ...
    const tryConnect = () => {
      ...
      transport.OnConnected.Add(() => {
        if (!resolved) {
          resolved = true;
          resolve2(new UeIpcSocket(transport));
        }
      });
...
describe("rpcServer", () => {
  ...
  let nodeProcessId;
  beforeAll(async () => {
    onClientConnect = new Emitter();
    server = new IPCServer(onClientConnect.event);
    ...
    const protocol = new Protocol(socket);
    onDidClientDisconnect = new Emitter();
