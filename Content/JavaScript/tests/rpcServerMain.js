...
var require_dist = __commonJS({
  "../universe-lib/dist/index.js"(exports2, module2) {
    ...
    var Protocol2 = class extends Disposable {
      _socket;
      _socketWriter;
      _socketReader;
    ...
    var NetIPCClient = class _NetIPCClient extends IPCClient {
      ...
      static fromSocket(socket, id2) {
        return new _NetIPCClient(new Protocol2(socket), id2);
      }
      get onDidDispose() {
    ...
    var NodeIPCServer = class _NodeIPCServer extends IPCServer2 {
      static toClientConnectionEvent(server) {
        ...
        return Event.map(onConnection, (socket) => ({
          protocol: new Protocol2(new NodeSocket(socket, "ipc-server-connection")),
...
// src/rpcServerMain.ts
var UE3 = __toESM(require("ue"));

// src/ipc/testRpcServer.ts
...
// src/ipc/testRpcServer.ts
async function testPuertsAsServer() {
  console.log("[testRpcServer] \u5F00\u59CB\uFF1APuerTS \u4F5C\u4E3A Server \u7B49\u5F85 Node.js Client");
  const onClientConnect = new import_lib3.Emitter();
  const server = new import_lib3.IPCServer(onClientConnect.event);
  const service = new CalculatorService();
  server.registerChannel(CHANNEL_NAME, createCalculatorServerChannel(service));
  const transport = new UE2.IPCTransport();
  ...
  const onDidClientDisconnect = new import_lib3.Emitter();
...
async function main() {
  ...
  UE3.PuertsTestHelper.MarkTestDone(exitCode);
}
main();
//# sourceMappingURL=rpcServerMain.js.map
